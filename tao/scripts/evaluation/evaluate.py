...
def main():
    ...
    if args.output_dir:
        ...
        if tensorboard_dir.exists():
            raise ValueError(
                'Tensorboard dir already exists, not evaluating.')
    ...
    area_index = tao_eval['tao_eval'].params.area_rng_lbl.index('all')
    time_index = tao_eval['tao_eval'].params.time_rng_lbl.index('all')
    category_aps = np.mean(
        tao_eval['tao_eval'].eval['precision'][:, :, :, area_index, time_index],
        axis=(0, 1))
    for i, cat in enumerate(tao_eval['tao_eval'].params.cat_ids):
        print(f'category_id: {cat}, ap:{category_aps[i]}')
