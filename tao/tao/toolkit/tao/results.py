...
class TaoResults(Tao):
    def __init__(self, tao_gt, results, max_dets=300, filter_bckg=True, filter_iou_thresh=0.5):
        ...
        self.filter_iou_thresh = filter_iou_thresh
        ...
        if filter_bckg:
            result_anns = self.filter_ignore_predictions(result_anns, tao_gt.dataset['annotations'])
    ...
    def filter_frame(self, preds, gts):
        filtered = []
        ignore_anns = []
        for pred in preds:
            iou = 0
            ...
            for gt in gts:
                if gt['category_id'] > 2:
                    continue
                if gt['ignore'] == 1 and gt['id'] not in ignore_anns:
                    ignore_anns.append(gt['id'])
                i, u = bb_intersect_union(pred['bbox'], gt['bbox'])
                if u == 0:
                    continue
                iou_ = i / u
                if iou_ > iou:
            ...
            if match is None or match['ignore'] == 0 or iou <= self.filter_iou_thresh:
                filtered.append(pred)
        
        return filtered, len(ignore_anns)


    def filter_ignore_predictions(self, preds, gts):
        print("Filtering predictions that fall on ignore regions")
        filtered = []
        if 'ignore' not in gts[0]:
            return preds
        img_pred = defaultdict(list)
        for pred in preds:
            img_pred[pred["image_id"]].append(pred)

        img_gts = defaultdict(list)
        for gt in gts:
            img_gts[gt["image_id"]].append(gt)

        cnt_ignore = 0
        for img_id in img_pred.keys():
            preds_frame = img_pred[img_id]
            gts_frame = img_gts[img_id]
            preds_frame, cnt = self.filter_frame(preds_frame, gts_frame)
            cnt_ignore += cnt
            filtered.extend(preds_frame)

        print("%d predictions have been filtered out" % (len(preds) - len(filtered)))
        print("%d ignore annotations were provided" % cnt_ignore)
