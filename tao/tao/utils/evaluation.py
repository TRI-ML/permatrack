...
_C.THRESHOLD = -1.0
#use ignore lables to skip certain gt tracks
_C.USE_GT_IGNORE = True
#remove detections that overalp with ignored tracks
_C.FILTER_BCK_PREDS = True
_C.FILTER_IOU_THRSH = 0.5
...
def evaluate(annotations, predictions, cfg, logger=logging.root):
    ...
    results = TaoResults(tao, results, filter_bckg=cfg.FILTER_BCK_PREDS, filter_iou_thresh=cfg.FILTER_IOU_THRSH)
    if cfg.ORACLE.TYPE != 'none':
        results = apply_oracle(tao,
                               ...
                               logger=logger)
    tao_eval = TaoEval(tao, results, iou_3d_type=cfg.IOU_3D_TYPE, use_gt_ignore=cfg.USE_GT_IGNORE)
    if cfg.CATEGORY_AGNOSTIC:
...
def log_eval(eval_info,
    ...
    def copy_tao_eval(tao_eval):
        eval_copy = TaoEval(tao_eval.tao_gt, tao_eval.tao_dt, use_gt_ignore=cfg.USE_GT_IGNORE)
